...
def question(request, question_id):
    ...
    if request.method == 'POST':
        ...
        if answer_form.is_valid():
            f = answer_form.save(commit=False, user=request.user)
    ...
    return render(request, "question_detail.html", {"answer_form": answer_form, "question": Question.objects.get(id=question_id), "answers": page_obj, "tags": Tag.objects.get_best(), "users": Profile.objects.get_best()})
...
def ask(request):
    ...
    return render(request, "ask.html", {"form": question_form, "tags": Tag.objects.get_best(), "users": Profile.objects.get_best()})
...
def settings(request):
    ...
    if request.method == 'POST':
        settings_form = SettingsForm(user, data=request.POST, files = request.FILES)
